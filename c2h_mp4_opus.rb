...
chatTxt.split("\r\n").each { |messageLine|
  ...
  timestampString = messageLine[1..endOfTimestamp-1].strip
...
messages.each_key { |date|
  ...
  messages[date].each { |message|
    if message["senderId"] and !message["attachment"]
      #
      # Regular user message or video.
      #
      if(message["text"].match('.mp4'))
        # Video.
        indexHtmlFile.puts("<p>")
        indexHtmlFile.puts("<b>" + uniToHtml(message["senderId"]) + "</b><br>")

        textMessage = message["text"]
        mp4Match = textMessage.match('.mp4')
        endOfMp4Text = mp4Match.end(0)
        mp4FileString = textMessage[0..endOfMp4Text-1].strip

        indexHtmlFile.puts("<video controls=\"\">")
        indexHtmlFile.puts("<source src=\"" + mp4FileString.to_s + "\" type=\"" + "video/mp4" + "\"/>" + "</video>")
      ...
      elsif attachmentFileType == "opus"
        inputFileName = inputDir.join(attachmentFileName)
        outputFileName = outputDir.join(attachmentFileName)
        # IO.copy_stream(inputFileName, outputFileName)
        indexHtmlFile.puts("<p>")
        indexHtmlFile.puts("<b>" + uniToHtml(message["senderId"]) + "</b><br>")
        indexHtmlFile.puts("<audio controls=\"\">")
        indexHtmlFile.puts("<source src=\"" + attachmentFileName.to_s + "\" type=\"" + "audio/ogg; codecs=opus" + "\"/>" + "</audio>")
        indexHtmlFile.puts("</p>")
      end
